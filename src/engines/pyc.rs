...
impl<'a> MarshalReader<'a> {
    ...
    fn i32(&mut self) -> Result<i32, String> {
        Ok(self.r.le_u32()? as i32)
    }

    fn i64(&mut self) -> Result<i64, String> {
        Ok(self.r.le_u64()? as i64)
    }

    fn len(&mut self, limit: usize) -> Result<usize, String> {
        let value = self.i32()?;
        if value < 0 {
            return Err("negative marshal length".to_owned());
        }
        let value = usize::try_from(value).map_err(|_| "marshal length overflow".to_owned())?;
        if value > limit {
            return Err("marshal object exceeds safety limit".to_owned());
        }
        Ok(value)
    }
    ...
    fn string_obj(&mut self, short: bool) -> Result<String, String> {
        let len = if short {
            usize::from(self.r.u8()?)
        } else {
            self.len(64 * 1024 * 1024)?
        };
    ...
    fn parse(&mut self) -> Result<Obj, String> {
        if self.depth > 200 {
            return Err("marshal nesting exceeds safety limit".to_owned());
        }
        self.depth += 1;
        ...
        let value = match tag {
            ...
            b'l' => {
                let n = self.i32()?;
                let count = usize::try_from(n.unsigned_abs())
                    .map_err(|_| "marshal long too large".to_owned())?;
                if count > 1_000_000 {
                    return Err("marshal long exceeds safety limit".to_owned());
                }
            ...
            b'{' => {
                ...
                for _ in 0..1_000_000 {
                    let key = self.parse()?;
                    if matches!(key, Obj::Other(ref s) if s == "NULL") {
                        break;
                    }
            ...
            b'r' => {
                let index = usize::try_from(self.r.le_u32()?)
                    .map_err(|_| "marshal ref overflow".to_owned())?;
                self.refs
                    .get(index)
                    .cloned()
                    .ok_or_else(|| "invalid marshal reference".to_owned())?
            }
...
fn obj_repr(obj: &Obj) -> String {
    match obj {
        ...
        Obj::Tuple(v) => format!(
            "({})",
            v.iter().map(obj_repr).collect::<Vec<_>>().join(", ")
        ),
        Obj::List(v) => format!(
            "[{}]",
            v.iter().map(obj_repr).collect::<Vec<_>>().join(", ")
        ...
        Obj::Dict(v) => format!(
            "{{{}}}",
            v.iter()
                .map(|(k, val)| format!("{}: {}", obj_repr(k), obj_repr(val)))
                .collect::<Vec<_>>()
                .join(", ")
        ),
...
fn render_code(code: &CodeObj, indent: &str, top_level: bool) -> String {
    ...
    if top_level {
        ...
        let _ = writeln!(
            out,
            "# file: {}  first line: {}",
            code.filename, code.firstlineno
        );
        let _ = writeln!(
            out,
            "# stacksize={} flags=0x{:x}\n",
        ...
        let _ = writeln!(
            out,
            "{indent}    # qualname={} posonly={} kwonly={}",
            code.qualname, code.posonlyargcount, code.kwonlyargcount
        );
    }
    ...
    if !code.consts.is_empty() {
        ...
        for (i, value) in code.consts.iter().enumerate() {
            if !matches!(value, Obj::Code(_)) {
                let _ = writeln!(out, "{body_indent}#   const[{i}] = {}", obj_repr(value));
            }
        }
    }
    if !code.names.is_empty() {
        let _ = writeln!(out, "{body_indent}# names: {}", code.names.join(", "));
    }
    ...
    for (offset, pair) in code.code.chunks(2).enumerate() {
        ...
        let detail = match name {
            "LOAD_CONST" => code
                ...
                .map(obj_repr)
                .unwrap_or_default(),
            "LOAD_NAME" | "STORE_NAME" | "LOAD_GLOBAL" | "LOAD_ATTR" | "STORE_ATTR"
            | "IMPORT_NAME" | "IMPORT_FROM" => code
                .names
                .get(usize::from(arg))
                .cloned()
                .unwrap_or_default(),
            "LOAD_FAST" | "STORE_FAST" | "DELETE_FAST" => code
                .locals
                .get(usize::from(arg))
                .cloned()
                .unwrap_or_default(),
        ...
        if name == "OP" {
            let _ = writeln!(
                out,
                "{body_indent}#   {:04x}: OP_{op:03} arg={arg} {detail}",
                offset * 2
            );
        } else {
            let _ = writeln!(
                out,
                "{body_indent}#   {:04x}: {name:<32} {arg:<3} {detail}",
                offset * 2
            );
        }
    }
    ...
    for child in nested {
        out.push_str(&render_code(child, indent, false));
        out.push('\n');
    }
    if !top_level && code.code.is_empty() {
        let _ = writeln!(out, "{body_indent}pass");
    }
    out
}
...
fn pyc_header(data: &[u8]) -> Result<String, String> {
    if data.len() < 8 {
        return Err("pyc file is too short".to_owned());
    }
    let magic = u16::from_le_bytes([data[0], data[1]]);
    let flags = if data.len() >= 8 {
...
fn try_parse(data: &[u8], offset: usize, modern: bool) -> Result<CodeObj, String> {
    let payload = data
        .get(offset..)
        .ok_or_else(|| "pyc header exceeds file".to_owned())?;
