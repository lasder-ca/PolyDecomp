...
fn pe_has_clr(data: &[u8]) -> bool {
    ...
    let Some(pe) = le_u32(data, 0x3c).and_then(|v| usize::try_from(v).ok()) else {
        return false;
    };
    if data.get(pe..pe + 4) != Some(b"PE\0\0") {
        return false;
    }
    let Some(opt_size) = le_u16(data, pe + 20).map(usize::from) else {
        return false;
    };
    let opt = pe + 24;
    let Some(magic) = le_u16(data, opt) else {
        return false;
    };
    let dir = match magic {
        0x10b => opt + 96,
    ...
    if opt_size < dir.saturating_sub(opt).saturating_add(15 * 8) {
        return false;
    }
...
pub fn detect(path: &Path) -> Result<Detection, String> {
    ...
    let data =
        prefix(path).map_err(|error| format!("failed to read {}: {error}", path.display()))?;
    let ext = path
        .extension()
    ...
    Ok(Detection {
        ...
        description: "Unknown format; built-in structural analysis is available".to_owned(),
        confidence: 0.20,
    })
}
...
mod tests {
    ...
    fn detects_java_class() {
        ...
        assert_eq!(
            detect(file.path()).expect("detect").kind,
            FileKind::JvmClass
        );
    }
